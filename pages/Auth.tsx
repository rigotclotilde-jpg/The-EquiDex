...
export const Auth: React.FC = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        await login(loginEmail || 'sophie.d@email.com', 'cavalier');
        navigate('/rider-dashboard');
    };

    const handleRegister = async (e: React.FormEvent) => {
        e.preventDefault();
        // Logique d'inscription simulée (utilise login pour la démo)
        await login('newuser@demo.com', regType);
        if (regType === 'cavalier') {
            navigate('/rider-dashboard');
        } else {
                ...
                <div className="flex-1 bg-white p-8 border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300 rounded-lg">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6 font-sans">
                        ...
                        <button 
                            type="submit" 
                            disabled={isLoading}
                            className="w-full bg-black text-white py-3 uppercase tracking-widest text-sm font-medium hover:bg-gray-800 transition-colors rounded-lg shadow-md flex items-center justify-center gap-2"
                        >
                            {isLoading ? <Loader2 className="animate-spin" size={20} /> : 'Se Connecter (Démo)'}
                        </button>
                    </form>
                ...
                <div className="flex-1 bg-white p-8 border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300 rounded-lg">
                    ...
                    {regType === 'cavalier' && (
                        <form onSubmit={handleRegister} className="space-y-5 font-sans animate-fade-in">
                            ...
                            <button type="submit" disabled={isLoading} className="w-full bg-black text-white py-3 uppercase tracking-widest text-sm font-medium hover:bg-gray-800 transition-colors rounded-lg shadow-md flex items-center justify-center gap-2">
